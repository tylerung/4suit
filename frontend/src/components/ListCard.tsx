...
import { useUser, useVenueMap } from '../lib/directory'
import { timeAgo } from '../lib/geo'
import './ListCard.css'
import Icon from './Icon'

export default function ListCard({ list, showOwner = false }: { list: VenueList; showOwner?: boolean }) {
  const owner = useUser(showOwner ? list.ownerId : null)
  const venues = useVenueMap()
  const preview = list.venueIds.slice(0, 3)
