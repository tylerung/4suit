...
interface MediaUrl {
  url: string
  /** True once the browser has told us the thumbnail did not load. */
  missing: boolean
  onError: () => void
}

/**
 * A just-uploaded attachment is already addressable on the API, so the
 * thumbnail has a URL immediately. What it does not have is a guarantee the
 * bytes will load, which only the element can report.
 */
  ...
  return (
    <li className="mp-tile" data-selected={selected ? 'true' : 'false'}>
      ...
      >
        ...
        {!missing && (
          isImage ? (
            <img className="mp-media" src={url} alt={item.alt} onError={onError} />
          ) : item.posterUri !== null ? (
            <img className="mp-media" src={item.posterUri} alt="" onError={onError} />
          ) : (
            <video
              className="mp-media"
              src={url}
    ...
    try {
      for (let i = 0; i < accepted.length; i += 1) {
        ...
        try {
          ...
          if (!aliveRef.current) {
            // The bytes are already uploaded but nothing will ever reference
            // them now, so drop them rather than orphan the file.
            void deleteMedia([added])
            return
          }
          next = [...next, added]
          onChange(next)
