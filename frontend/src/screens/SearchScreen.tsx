...
import { rankVenues, search, suggestedUsers, type SearchResults } from '../lib/api'
...
import type { RankedVenue, User } from '../types'
...
const NO_RESULTS: SearchResults = { venues: [], users: [], posts: [], lists: [] }
const NO_ROOMS: RankedVenue[] = []
const NO_USERS: User[] = []
...
export default function SearchScreen() {
  ...
  /* One request covers all four kinds of result: the server searches rooms,
     people, posts and lists together and scopes each to what this viewer is
     allowed to see. */
  const { data: results, loaded } = useQuery<SearchResults>(
    () => search(query, origin),
    [query, viewerId, origin.lat, origin.lng],
    NO_RESULTS,
  )

  const { data: trending } = useQuery<RankedVenue[]>(
    async () => (await rankVenues({ origin, sort: 'rating' })).slice(0, 5),
    [origin.lat, origin.lng],
    NO_ROOMS,
  )

  const { data: suggested } = useQuery<User[]>(
    () => suggestedUsers(5),
    [viewerId],
    NO_USERS,
  )
  ...
  return (
    <>
      ...
      {query === '' ? (
        ...
        <p className="screen-loading faint" role="status">Looking…</p>
      ) : counts.all === 0 ? (
        <div className="srch-empty">
          <EmptyState
            icon="🃏"
