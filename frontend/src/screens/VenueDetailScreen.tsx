...
import {
  ...
  type FeedItem, type RankedVenue, type Rating, type Venue, type VenueList, type VenueStats,
} from '../types'
import {
  createList, deleteRating, getVenue, getVenueStats, listListsByUser,
...
/**
 * One review. It resolves its own author rather than being handed one: the API
 * returns ratings by author id, and the account cache behind useUser collapses
 * a screen full of reviews by the same regular into a single request.
 */
function ReviewCard(
  { rating, mine, actions }:
  { rating: Rating; mine: boolean; actions?: ReactNode },
) {
  const author = useUser(rating.userId)
...
export default function VenueDetailScreen() {
  ...
  const { currentUser, origin, originLabel } = useApp()
  ...
  const { data, loaded } = useQuery<VenueData | null>(
    async () => {
      if (!venueId) return null
      const venue = await getVenue(venueId)
      if (!venue) return null

      // Everything else about the room is independent, so it goes out at once
      // rather than as a waterfall of six round trips.
      const [stats, ranked, ratings, posts, lists] = await Promise.all([
        getVenueStats(venue.id),
        rankVenues({ origin: { lat: originLat, lng: originLng } }),
        listRatingsForVenue(venue.id),
        listPostsForVenue(venue.id),
        meId ? listListsByUser(meId) : Promise.resolve([]),
      ])

      const row = ranked.find((r) => r.venue.id === venue.id) ?? null
      // "Nearby" is measured from this room, not from the viewer, so the
      // distances come back out of the ranked rows and are recomputed here.
      const nearby = ranked
      ...
      return { venue, stats, row, nearby, ratings, posts, lists }
    },
    [venueId, originLat, originLng, meId],
    null,
  )
  ...
  if (!loaded) {
    return (
      <>
        ...
        <p className="screen-loading faint" role="status">Reading the room…</p>
      </>
    )
  }
  ...
  const removeMyReview = () => {
    ...
    void deleteRating(myRating.id)
      .then(() => showToast('Review deleted'))
      .catch((err: unknown) => showToast(err instanceof Error ? err.message : 'Could not delete that.'))
  }

  const createAndAdd = () => {
    const name = newListName.trim()
    if (!meId || !name) return
    void createList({ name, isPublic: newListPublic, venueIds: [venue.id] })
      .then(() => {
        setNewListName('')
        setNewListPublic(false)
        showToast(`Started “${name}”`)
      })
      .catch((err: unknown) => showToast(err instanceof Error ? err.message : 'Could not start that list.'))
  }
            ...
            {ratings.length === 0 ? (
              ...
              <div className="vd-reviews">
                ...
                {otherRatings.map((rating) => (
                  <ReviewCard key={rating.id} rating={rating} mine={false} />
                ))}
              </div>
            )}
          </>
                ...
                {lists.map((list) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        void toggleVenueInList(list.id, venue.id).then((added) => {
                          showToast(added
                            ? `Added to ${list.name}`
