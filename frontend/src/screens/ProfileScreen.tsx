...
const NO_POSTS: FeedItem[] = []
const NO_RATINGS: Rating[] = []
const NO_LISTS: VenueList[] = []
const NO_USERS: User[] = []
...
export default function ProfileScreen() {
  ...
  /* /profile shows whoever is signed in, and that record is already in hand —
     asking the API for it again would only introduce a copy that can lag behind
     the one the rest of the app is using, which is exactly what happens the
     moment you edit your own profile. Only /u/:username is a lookup. */
  const { data: lookedUp, loaded: lookupLoaded } = useQuery<User | null>(
    () => (username ? getUserByUsername(username) : Promise.resolve(null)),
    [username],
    null,
  )
  const user = username ? lookedUp : currentUser
  const userLoaded = username ? lookupLoaded : true
  const isMe = user !== null && user.id === viewerId
  /* Whether a private account's activity is readable is decided on the server —
     the endpoints below simply come back empty when it is not. This is the same
     question asked locally, only to choose between the locked view and the
     tabs; it never stands in for the server's answer. */
  const canView = user !== null
    && (!user.isPrivate || isMe || (viewerId !== null && user.followerIds.includes(viewerId)))
  ...
  const { data: posts } = useQuery<FeedItem[]>(
    () => (userId && canView ? listPostsByUser(userId) : Promise.resolve(NO_POSTS)),
    [userId, canView, viewerId],
    NO_POSTS,
  )
  const { data: ratings } = useQuery<Rating[]>(
    () => (userId && canView ? listRatingsByUser(userId) : Promise.resolve(NO_RATINGS)),
    [userId, canView],
    NO_RATINGS,
  )
  const { data: lists } = useQuery<VenueList[]>(
    () => (userId && canView ? listListsByUser(userId) : Promise.resolve(NO_LISTS)),
    [userId, canView, viewerId],
    NO_LISTS,
  )

  /* Followers, following and pending requests come from one call, and the
     pending list is only ever populated for your own account — the server does
     not hand somebody else's approval queue to a visitor. */
  const { data: connections } = useQuery(
    () => (userId
      ? getConnections(userId)
      : Promise.resolve({ followers: NO_USERS, following: NO_USERS, pendingFollowers: NO_USERS })),
    [userId, isMe],
    { followers: NO_USERS, following: NO_USERS, pendingFollowers: NO_USERS },
  )
  const requests = isMe ? connections.pendingFollowers : NO_USERS
  const peopleUsers = !people
    ? NO_USERS
    : people === 'followers' ? connections.followers : connections.following
  ...
  if (!userLoaded) {
    return (
      <div className="prof-screen">
        ...
        <p className="screen-loading faint" role="status">Looking them up…</p>
      </div>
    )
  }
  ...
  const homeVenue = allVenues.find((v) => v.id === profile.homeVenueId) ?? null
  ...
  const saveEdit = () => {
    ...
    setEditError(null)
    // Validation — a taken handle, an oversized photo — is the server's answer,
    // and its message is what goes in front of the user.
    void updateProfile(profile.id, {
      ...
      .catch((err: unknown) => {
        setEditError(err instanceof Error ? err.message : 'That profile could not be saved.')
      })
  }
  ...
  const submitList = () => {
    ...
    void createList({
      ...
      .catch((err: unknown) => {
        setListError(err instanceof Error ? err.message : 'That list could not be started.')
      })
  }
              ...
              <div className="prof-request" key={r.id}>
                ...
                <div className="prof-request-actions">
                  <button
                    ...
                    onClick={() => {
                      void acceptFollowRequest(profile.id, r.id)
                  ...
                  <button
                    ...
                    onClick={() => {
                      void declineFollowRequest(profile.id, r.id)
            ...
            {tab === 'ratings' && (
              ratings.length === 0 ? (
                ...
                <div className="prof-ratings">
                  {ratings.map((r) => {
                    const venue = allVenues.find((v) => v.id === r.venueId)
