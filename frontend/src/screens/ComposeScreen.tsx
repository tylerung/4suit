...
import { createPost, getMyRating, saveRating } from '../lib/api'
...
export default function ComposeScreen() {
  ...
  const paramVenueId = searchParams.get('venue')
  ...
  const [subscores, setSubscores] = useState<Subscores>({ ...EMPTY_SUBSCORES })
  ...
  /* The room list is small and unchanging, so the composer filters the cached
     copy rather than asking the API on every keystroke. Distance is measured
     here too: the device's coordinates are the one thing the server does not
     have, and should not be sent just to sort a picker. */
  ...
  const { data: existing } = useQuery<Rating | null>(
    () => (me && venueId ? getMyRating(venueId) : Promise.resolve(null)),
    [me?.id, venueId],
    null,
  )

  /* A deep link from a room's page (/compose?venue=…&mode=rating) opens with
     that room's existing rating already in the form. It used to be read during
     the first render; now it arrives a moment later, so it is applied when it
     lands — once, and never over anything the user has typed in the meantime. */
  const seeded = useRef(false)
  useEffect(() => {
    if (seeded.current || initialMode !== 'rating' || !paramVenueId) return
    if (!existing || existing.venueId !== paramVenueId) return
    seeded.current = true
    setSubscores((current) =>
      Object.values(current).some((v) => v > 0) ? current : { ...existing.subscores })
    setStakes((current) => (current.trim() ? current : existing.stakesPlayed))
    setBody((current) => (current.trim() ? current : existing.review))
  }, [existing, initialMode, paramVenueId])
  ...
  }, [venues, venueQuery, originLat, originLng])
    ...
    return (
      <>
        ...
        <EmptyState
          ...
          body="4suit needs to know whose session this is."
          action={
  ...
  const applyRating = (r: Rating) => {
    ...
    setBody((current: string) => (current.trim() ? current : r.review))
  }
  ...
  const selectVenue = (v: Venue) => {
    ...
    // Clear first: the old room's scores must not sit on screen against the new
    // room's name while the server is still being asked about it.
    setSubscores({ ...EMPTY_SUBSCORES })
    void getMyRating(v.id).then((prior) => {
      if (!prior) return
  ...
  const changeMode = (next: PostKind) => {
    ...
    if (next === 'rating' && venueId) {
      void getMyRating(venueId).then((prior) => { if (prior) applyRating(prior) })
    }
  }
  ...
  const handleSubmit = () => {
    ...
    const compose = async (): Promise<string> => {
      if (mode === 'rating') {
        if (!venue) throw new Error('No room selected.')
        // The rating is saved first: the post quotes its id and its score, so a
        // post that went up first could name a rating that failed to save.
      ...
      if (mode === 'session') {
        if (!venue || netNum === null || !hoursValid) throw new Error('Incomplete session.')
    ...
    compose()
      .then((postId) => {
        showToast('Posted')
        // Cleared so the unmount below does not treat the attachments as an
        // abandoned draft and delete what was just posted.
        setAttachments([])
        navigate(`/post/${postId}`)
      })
      .catch((err: unknown) => {
        showToast(err instanceof Error ? err.message : 'That post did not go up.')
      })
      .finally(() => setSubmitting(false))
  }

  /* Attachments are uploaded the moment they are picked, before any post
     exists. Walking away without posting would strand those files with nothing
     referencing them, so an abandoned draft takes its media with it. */
  const discardDraft = () => {
