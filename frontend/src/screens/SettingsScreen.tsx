...
import { resetDemoData, updateProfile } from '../lib/api'
...
export default function SettingsScreen() {
  const { currentUser: me, signOutNow } = useApp()
  ...
  const pendingCount = me ? me.pendingFollowerIds.length : 0
  ...
  const togglePrivate = () => {
    ...
    void updateProfile(me.id, { isPrivate: next })
      .then(() => {
        setError(null)
  ...
  const toggleRole = (role: Role) => {
    const roles = me.roles.includes(role)
      ...
      : [...me.roles, role]
    void updateProfile(me.id, { roles })
      .then(() => setError(null))
      .catch((err: unknown) => {
        setError(err instanceof Error ? err.message : 'That change could not be saved.')
      })
  ...
  const handleSignOut = () => {
    void signOutNow().then(() => navigate('/signin', { replace: true }))
  }

  const handleReset = () => {
    const ok = window.confirm(
      'Reset 4suit to the seeded demo data?\n\n'
        + 'Every post, rating, list and follow on this server is deleted, for every account. '
    ...
    void resetDemoData()
      ...
      .catch((err: unknown) => {
        setError(err instanceof Error ? err.message : 'The demo data could not be restored.')
      })
  }
    ...
    <>
      ...
      <div className="set-body">
            ...
            <div className="set-switch-text">
              ...
              <p className="field-hint set-hint">
                {me.isPrivate
                  ...
                  : 'Anyone on 4suit can see your posts, ratings and lists. Turn this on and new followers have to be approved first.'}
        ...
        <section className="card set-card" aria-labelledby="set-h-theme">
          ...
          <p className="field-hint set-hint">
            4suit runs dark — it gets used in card rooms at 2am. System follows your device and
        ...
        <section className="card set-card" aria-labelledby="set-h-data">
          ...
          <p className="field-hint set-hint">
            4suit keeps the whole demo — accounts, posts, ratings, lists and follows — on its
            server. Signing out just empties the seat; nothing is deleted.
          </p>
          ...
          <p className="field-hint set-hint">
            Reset throws away everything anybody has written and deals the seeded rooms, players
            and posts again — for every account, not just yours. There is no undo.
          </p>
        </section>
        ...
        <section className="card set-card" aria-labelledby="set-h-about">
          ...
          <p className="set-about">
            4suit is a demo. There are no passwords and no real accounts — you pick a seat at
            the door — but everything you write goes to the 4suit API and MongoDB behind it, so
            everyone signed in is looking at the same rooms, posts and ratings.
          </p>
