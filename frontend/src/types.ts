/**
 * 4suit — shared domain model.
 *
 * These are the shapes the API sends and accepts, so the server keeps a
 * matching copy in `backend/src/types.ts`: a change here is a change there.
 * What lives here and not there is presentational — role labels, amenity
 * labels, the metro picker; what lives there and not here is the vocabulary
 ...
 * makeAvatarPhoto in lib/media.ts for why they are not uploaded as files).
 */
...
/**
 * An attachment's metadata. Deliberately holds no bytes: the pixels live on the
 * server under `id` and are served from /api/media/:id, which is the URL
 * lib/media.ts builds at render time. Seeded demo media is the exception — it
 * is a handful of bytes of inline SVG, so it rides along in `dataUri`.
 */
export interface MediaItem {
  id: string
  kind: MediaKind
  mime: string
  /** Inline source for seeded demo media; null when the bytes are on the server. */
  dataUri: string | null
  width: number
  height: number
  /** Videos only. */
...
/** Attachment limits, surfaced in the composer; the server enforces them too. */
