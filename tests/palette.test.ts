// Palette guard. Railbird is built from four colours plus one reserved alert
...
import { USERS, VENUES } from '../src/lib/seed'
...
console.log('\n— data is on the palette too —')
{
  ...
  check('every seeded user has a palette avatar tone',
    USERS.every((u) => tones.has(u.avatarTone)), USERS.map((u) => u.avatarTone).join(','))
  check('seeded tones actually vary', new Set(USERS.map((u) => u.avatarTone)).size === tones.size)
